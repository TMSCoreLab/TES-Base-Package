...
for m = 1:length(name)
    load(name{m}); 
    P = 1e-3*P;     %  only if the original data were in mm!
    tt = [tt; t+size(PP, 1)];
    PP = [PP; P];
...
disp([newline 'Combined mesh saved in ' num2str(toc) ' s']);
...
save(NewName, 'tneighbor', 'EC', 'PC', 'integralpd', 'ineighborE', '-v7.3');
disp([newline 'Neighbor integrals saved in ' num2str(toc) ' s']);